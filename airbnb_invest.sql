...
WITH airbnb_revenue AS (
    SELECT
        neighbourhood AS neighbourhood,
        Borough AS borough,
        SUM(price)*10/COUNT(neighbourhood) AS avg_daily_price,
        AVG(365 - availability_365) AS avg_occupied_nights,
        AVG(price*10 * (365 - availability_365)) AS avg_annual_revenue,
        COUNT(*) AS total_listings,
        -- Calcul de part de marché par groupe de quartiers
        ROUND(
            COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (PARTITION BY Borough), 2
...
property_costs AS (
    SELECT 
        ...
        AVG(`Sale Price` / NULLIF(`Gross Square Feet`, 0)) * 600 AS estimated_apartment_cost
    FROM house_listings
    WHERE `Residential Units` > 0 AND `Gross Square Feet` > 0 AND `Sale Date` >= '2019-01-01' AND `Sale Price` > 100000
    GROUP BY Neighborhood
),
...
roi_calculation AS (
    SELECT 
        ...
        pc.avg_property_price AS acquisition_cost,
...
-- STEP 4: ROI percentage of total
-- STEP 1 to 3 inclus
 WITH airbnb_revenue AS (
    SELECT
        neighbourhood AS neighbourhood,
